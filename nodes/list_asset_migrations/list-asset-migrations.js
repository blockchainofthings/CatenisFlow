...
module.exports = function(RED) {
    function listAssetMigrations(config) {
        ...
        node.on('input', function(msg) {
            ...
            if (util.checkNonNullObject(msg.payload)) {
                ...
                if (util.checkNonNullObject(msg.payload.selector)) {
                    ...
                    if (msg.payload.selector.direction !== null) {
            ...
            ctnApiClient.listAssetMigrations(selector, limit, skip, responseHandler.bind(node, msg));
        });
    }

    RED.nodes.registerType("list asset migrations", listAssetMigrations);

    RED.httpAdmin.post("/catenis.listassetmigrations/:id", RED.auth.needsPermission("catenis.listassetmigrations"), function(req, res) {
        ...
        if (node != null) {
            try {
                ...
                res.sendStatus(500);
                node.error("Error sending message to node component in response to catenis.listassetmigrations/:id request: " + err);
            }
        } else {
            res.sendStatus(404);
        }
